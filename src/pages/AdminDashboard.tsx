...
import { Eye, Trash2, LogOut, Phone, MapPin, Wrench, Shield, Calendar, Search, Users } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-5 py-6 space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Total Leads", value: bookings.length, icon: Users, color: "bg-primary/10 text-primary" },
            { label: "Today", value: bookings.filter((b) => new Date(b.created_at).toDateString() === new Date().toDateString()).length, icon: Calendar, color: "bg-green-500/10 text-green-600" },
            { label: "This Week", value: bookings.filter((b) => { const d = new Date(b.created_at); const now = new Date(); const diff = (now.getTime() - d.getTime()) / (1000 * 60 * 60 * 24); return diff <= 7; }).length, icon: Wrench, color: "bg-amber-500/10 text-amber-600" },
            { label: "This Month", value: bookings.filter((b) => { const d = new Date(b.created_at); const now = new Date(); return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear(); }).length, icon: Shield, color: "bg-purple-500/10 text-purple-600" },
          ].map((stat) => (
        ...
        <div className="relative">
          ...
          <Input
            placeholder="Search by name, phone, appliance..."
            value={searchQuery}
